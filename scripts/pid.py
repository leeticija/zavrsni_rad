...
class PID:
    ...
    def compute(self, ref, meas):
        ...
        if self.firstPass:
            ...
            self.up = self.kp * error                           # setting proportional term of PID

            if self.ki == 0:
                self.ui = 0
            else:
                self.ui = self.ui_old + self.ki * error * dt    # setting integral term of PID

            self.ud = self.kd * de / dt                         # setting derivative term of PID
            ...
            self.error_old = error                          # save error for next step
